...
def main() -> None:
    ...
    p.add_argument("--method", choices=["bfs", "smc", "both"], default=None,
                   help="run only best-first (bfs) or only SMC, or both "
                        "(default: follows the DO_SMC module toggle)")
    ...
    # Rebase reconciliation: `main` added the `--method` CLI flag while this
    # branch added the `DO_SMC` module toggle. Both drive the same `methods`, so
    # the flag wins when given and otherwise falls back to DO_SMC.
    method = a.method if a.method is not None else ("both" if DO_SMC else "bfs")
    methods = {"bfs": ["enum"], "smc": ["smc"], "both": ["enum", "smc"]}[method]
