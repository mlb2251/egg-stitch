...
//! to `max(fv) + 1`, absence past it, absence for fv-empty classes, and that
//! returned ids are canonical post-rebuild.
...
#[test]
fn variants_built_up_to_max_fv_plus_one() {
    // `$1` has fv {1}, so max(fv)+1 = 2:
    //   shift s=1 → `$0`   (fv {0})    — still a positive free index
    //   shift s=2 → `$-1`  (fv {-1})   — first re-wrap variant
    //   s=3 is past `max(fv)+1`; deeper re-wraps can be lam-materialized at
    //   extraction time, so no variant is built.
    let mut eg: egg::EGraph<LamLang, StitchAnalysis> = egg::EGraph::default();
    eg.add_expr(&LamLang::parse_program("(lam (lam $1))").unwrap());
    ...
    let s2 = v.get(leaf, 2).expect("shift by 2 should be built (first re-wrap)");
