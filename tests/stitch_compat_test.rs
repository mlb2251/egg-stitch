...
/// (input, search): several tests run best-first on the same input (e.g. the
/// `if_branch_unify` cap variants), and a shared path would let parallel tests
/// race on the same file. Stem + search are kept only for human readability.
