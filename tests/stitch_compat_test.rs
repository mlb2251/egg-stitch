...
fn check_fixture(input: &str, extra_args: &[&str], check_pattern: bool) {
    ...
    // best_history is a dominance-only per-expansion log that best-first
    // populates and smc doesn't. Strip it symmetrically so the bf/smc equality
    // check measures search-result agreement, not trace-shape agreement.
