...
fn zero_abstractions() {
    ...
    let (library, _original_size, final_cost, final_rewritten) = multiple_step_search::<OpChildren, Op>(eg, root, ShiftedVariants::default(), &args(100, 0));
...
fn one_abstraction_rewritten_corpus() {
    let (eg, root) = load::<OpChildrenLanguage>();
    let (library, _original_size, _final_cost, final_rewritten) = multiple_step_search::<OpChildren, Op>(eg, root, ShiftedVariants::default(), &args(500, 1));
...
fn two_abstractions() {
    let (eg, root) = load::<OpChildrenLanguage>();
    let (library, original_size, final_cost, _final_rewritten) = multiple_step_search::<OpChildren, Op>(eg, root, ShiftedVariants::default(), &args(500, 2));
