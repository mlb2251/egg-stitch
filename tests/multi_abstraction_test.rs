...
fn zero_abstractions() {
    ...
    let (library, _original_size, final_cost, final_rewritten) = multiple_step_search::<OpChildren, Op>(SharedData::new(eg, root), &args(0));
...
fn one_abstraction_rewritten_corpus() {
    let (eg, root) = load::<OpChildrenLanguage>();
    let (library, _original_size, _final_cost, final_rewritten) = multiple_step_search::<OpChildren, Op>(SharedData::new(eg, root), &args(1));
...
fn two_abstractions() {
    let (eg, root) = load::<OpChildrenLanguage>();
    let (library, original_size, final_cost, _final_rewritten) = multiple_step_search::<OpChildren, Op>(SharedData::new(eg, root), &args(2));
