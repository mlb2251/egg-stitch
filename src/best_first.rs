...
pub fn best_first<F: LanguageFamily, O: StitchOp>(data: crate::shared::SharedData<F, O>, args: &crate::Args) -> BestFirstResult<F, O> {
    ...
    'search: loop {
        ...
        // mass). Per-factor (not summed) so high-usage patterns — many matches,
        // tiny 1-row factors — survive; only entangled commutativity-blowup
        // factors are pruned. See `SearchState::max_factor_weight`.
