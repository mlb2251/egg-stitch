...
fn run<F: egg_stitch::lang::LanguageFamily, O: StitchOp>(args: &Args) -> RunOutput {
    ...
    let (egraph, root, cost_before_rewrites, original_programs, shifted) = io::load_egraph::<F, O>(&args.input, args.rules.as_deref(), args.weights);
    println!("load_egraph took {:.3}s", load_start.elapsed().as_secs_f64());
    let (library, original_size, final_cost, final_rewritten) = multiple_step_search::<F, O>(egraph, root, shifted, args);
    RunOutput {
        library,
        original_size,
        final_cost,
