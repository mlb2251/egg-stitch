use crate::lang::{LanguageFamily, StitchAnalysis, StitchEgraph, StitchLanguage, StitchOp};
use anyhow::anyhow;
...
pub fn load_egraph<F: LanguageFamily, O: StitchOp>(filename: &str, rule_file: Option<&str>) -> (StitchEgraph<F, O>, egg::Id, usize) {
    ...
    let (egraph_before_rules, root) = programs_to_egraph::<F, O>(&exprs);
    ...
    let rules: Vec<egg::Rewrite<F::Apply<O>, StitchAnalysis<F, O>>> = match rule_file {
        Some(rule_file) => from_file(rule_file).expect("Failed to parse rules file"),
        None => vec![],
    };
    println!("loaded {} rules", rules.len());

    let mut runner: egg::Runner<F::Apply<O>, StitchAnalysis<F, O>> = egg::Runner::default();
...
pub fn egraph_from_programs<F: LanguageFamily, O: StitchOp>(programs: &[String], rule_file: Option<&str>) -> (StitchEgraph<F, O>, egg::Id) {
    let (egraph, root) = programs_to_egraph::<F, O>(programs);
    let rules: Vec<egg::Rewrite<F::Apply<O>, StitchAnalysis<F, O>>> = match rule_file {
...
fn programs_to_egraph<F: LanguageFamily, O: StitchOp>(programs: &[String]) -> (StitchEgraph<F, O>, egg::Id) {
    let mut egraph: StitchEgraph<F, O> = egg::EGraph::default();
    let expr_ids: Vec<egg::Id> = programs
        .iter()
        .map(|s| {
            let expr: egg::RecExpr<F::Apply<O>> = s.parse().unwrap_or_else(|_| panic!("Failed to parse expression: {s}"));
            egraph.add_expr(&expr)
        })
        .collect();
    let programs_node = <F::Apply<O> as FromOp>::from_op("programs", expr_ids).expect("Failed to create programs node");
...
fn extract_root_size<F: LanguageFamily, O: StitchOp>(egraph: &StitchEgraph<F, O>, root: egg::Id) -> usize {
