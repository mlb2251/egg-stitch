...
pub fn compute_size<F: LanguageFamily, O: StitchOp>(egraph: &StitchEgraph<F::Apply<O>>, root: Id, cache: &CostCache, scratch: &mut CostScratch, search_state: &SearchState<F, O>, check_slow: bool, ho_arity: &[u32]) -> usize {
    ...
    if check_slow {
        ...
        assert_eq!(egraph[root].data.fv, rewritten.egraph[rewritten.root].data.fv, "free-variable set diverges after rewrite: original {:?} != rewritten {:?}", egraph[root].data.fv, rewritten.egraph[rewritten.root].data.fv,);
    }
    final_size as usize
}
