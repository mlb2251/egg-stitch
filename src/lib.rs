    ...
    for abstraction_idx in 0..args.num_abstractions {
        ...
        match best {
            ...
            Some((best_cost, state)) => {
                // Re-derive the chosen candidate so display/rewrite use the same
                // optimised tuple the search settled on.
                let cache = cost::CostCache::new(&result_data.egraph, result_data.root);
                let mut scratch = cost::CostScratch::new(&result_data.egraph);
                let selection = cost::compute_cost_and_select::<F, O>(&result_data.egraph, result_data.root, &cache, &mut scratch, &state, false);
                let variable_indices = &selection.candidate.variable_indices;
...
fn apply_abstraction<F: LanguageFamily, O: StitchOp>(data: shared::SharedData<F, O>, state: &search::SearchState<F, O>, candidate: &cost::CostCandidate, fn_name: &str, rule_file: Option<&str>) -> (shared::SharedData<F, O>, Vec<String>) {
