    ...
    for abstraction_idx in 0..args.num_abstractions {
        ...
        match best {
            ...
            Some((best_cost, state)) => {
                let magnitudes = cost::compute_ho_magnitudes::<F, O>(&result_data.egraph, &state);
                ...
                let body_str = state.pattern.display_with_ho(&magnitudes);
                let lambda = state.pattern.display_as_lambda(&magnitudes);
...
fn apply_abstraction<F: LanguageFamily, O: StitchOp>(data: shared::SharedData<F, O>, state: &search::SearchState<F, O>, fn_name: &str, rule_file: Option<&str>) -> (shared::SharedData<F, O>, Vec<String>) {
    ...
    for m in &state.matches {
        for subst in &m.substs {
            let wrapped = cost::wrap_subst_args::<F, O>(&mut egraph, &subst.vars, &magnitudes, var_depth);
