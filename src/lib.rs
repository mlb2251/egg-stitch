    ...
    for abstraction_idx in 0..args.num_abstractions {
        ...
        match best {
            ...
            Some((best_cost, best)) => {
                ...
                let (next_data, rewritten_programs) = apply_abstraction::<F, O>(result_data, state, candidate, &fn_name, rule_file, args.iter_limit);
...
pub fn apply_abstraction<F: LanguageFamily, O: StitchOp>(data: shared::SharedData<F, O>, state: &search::SearchState<F, O>, candidate: &cost::CostCandidate, fn_name: &str, rule_file: Option<&str>, iter_limit: usize) -> (shared::SharedData<F, O>, Vec<String>) {
    ...
    let fresh = io::egraph_from_programs::<F, O>(&programs, rule_file, weights, iter_limit);
    (fresh, programs)
}
