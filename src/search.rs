...
impl<F: LanguageFamily, O: StitchOp> SearchState<F, O> {
    ...
    ///
    /// This isolates the commutativity-blowup signature — one factor whose slots
    /// are *entangled* (so it can't `decompose`) holding the exponentially many
    /// equivalent parse trees of a single coordinate. A genuinely high-*usage*
    /// pattern instead has many matches each with tiny 1-row factors, so its
    /// per-factor mass stays small and it survives the cap. Capping the *sum*
    /// over all rows would conflate the two; the per-factor max does not.
