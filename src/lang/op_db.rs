...
impl<O: StitchOp> StitchOp for OpDB<O> {
    /// Parses `"$<i32>"` as `Var(n)` (including negative indices like `$-2`,
    /// which represent re-wrap slots in shifted variants); anything else
    /// (including `"$foo"` with a non-numeric suffix) is delegated to the inner
    /// op type.
